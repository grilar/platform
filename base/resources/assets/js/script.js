class Grilar {
    constructor() {
        ...
        Grilar.handleCounterUp()
        Grilar.initMediaIntegrate()
        if (GrilarVariables && GrilarVariables.authorized === '0') {
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = GrilarVariables.languages.notices_msg.error
                    break
                case 'success':
                    messageHeader = GrilarVariables.languages.notices_msg.success
                    break
            }
        }
    ...
    static handleError(data) {
        if (typeof data.errors !== 'undefined' && !_.isArray(data.errors)) {
            Grilar.handleValidationError(data.errors)
        } else {
            if (typeof data.responseJSON !== 'undefined') {
                if (typeof data.responseJSON.errors !== 'undefined') {
                    if (data.status === 422) {
                        Grilar.handleValidationError(data.responseJSON.errors)
                    }
                } else if (typeof data.responseJSON.message !== 'undefined') {
                    Grilar.showError(data.responseJSON.message)
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Grilar.showError(item)
                        })
                    })
                }
            } else {
                Grilar.showError(data.statusText)
            }
        }
    }
            ...
            settings = $.extend(
                {
                    ...
                    format: '(%1 ' + GrilarVariables.languages.system.character_remain + ')',
                    pulse: true,
                    delay: 0,
                },
                settings
    ...
    static initResources() {
        ...
        Grilar.initDatePicker('.datepicker')
        if (jQuery().mCustomScrollbar) {
            Grilar.callScroll($('.list-item-checkbox'))
        }
    ...
    static initMediaIntegrate() {
        if (jQuery().rvMedia) {
            Grilar.gallerySelectImageTemplate = `<div class='list-photo-hover-overlay'>
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = Grilar.gallerySelectImageTemplate
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown((event) => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Grilar.onDragTool)
            $(window).on('mouseup', Grilar.onReleaseTool)
        })
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Grilar.onDragTool)
        $(window).off('mouseup', Grilar.onReleaseTool)
    }
